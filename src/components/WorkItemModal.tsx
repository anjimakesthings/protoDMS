...
export default function WorkItemModal({ item, initialDate, initialMode, onClose }: Props) {
  const { users, createWorkItem, updateWorkItem, deleteWorkItem, isReadOnly, getOrderByReference } = useApp()
  ...
  const [timeMode, setTimeMode] = useState<'allday' | 'interval'>(item?.scheduledTimeFrom ? 'interval' : 'allday')
  const [timeFrom, setTimeFrom] = useState(item?.scheduledTimeFrom ?? '')
  const [timeTo, setTimeTo] = useState(item?.scheduledTimeTo ?? '')
  ...
  function handlePrint() {
    ...
    // Wait for all images in the print root to load before printing
    const tryPrint = () => {
      ...
      if (pending.length === 0) {
        ...
        let loaded = 0
        pending.forEach(img => {
          img.addEventListener('load', () => { if (++loaded === pending.length) window.print() }, { once: true })
          img.addEventListener('error', () => { if (++loaded === pending.length) window.print() }, { once: true })
        })
      }
    }
  ...
  function handleSave() {
    ...
    const data = {
      ...
      assignedToUserIds,
      scheduledDate: scheduledDate ? new Date(`${scheduledDate}T00:00`).toISOString() : null,
      scheduledTimeFrom: scheduledDate && timeMode === 'interval' && timeFrom ? timeFrom : undefined,
      scheduledTimeTo: scheduledDate && timeMode === 'interval' && timeTo ? timeTo : undefined,
      ...
      <div className="modal-dialog">
        ...
        {mode === 'view' && item ? (
          <div className="space-y-4">
            ...
            {item.description && (
              <div>
                <div className="modal-field-label">Kommentar</div>
          ...
          {/* Kommentar */}
          <div className="mb-4">
            <label className="modal-field-label">Kommentar</label>
            <textarea
              className="modal-input"
              rows={3}
              placeholder="Kommentar av ärendet..."
              value={description}
        ...
        <div className="modal-footer">
          {mode === 'view' ? (
            <>
              <button className="btn-secondary" style={{ marginRight: 'auto' }} onClick={onClose}>Avbryt</button>
